...
\begin{align*}
  ...
  &=  \mname{val(Nil) + val(Nil)}    & \pnote{RHS of $P(0)$}
\end{align*}
So $P(0)$ holds.
...
\textcolor{red}{Induction step}: $n > 1$. Assume $P(2), P(3), ... , P(n-2), P(n-1)$. We must show $P(n)$.
  ...
  \bi
    ...
    \item[] $\mname{val(add(Join(u,Zero), Join(v,Zero))) = val(Join(add(u,v),Zero)) = 2 * Join(add(u,v)}.$

   \item[] $\mname{val(add(Join(u,One), Join(v,Zero))) = val(Join(add(u,v),One)) = 2 * Join(add(u,v) + 1}.$

    \item[] $\mname{val(add(Join(u,Zero), Join(v,One))) = val(Join(add(u,v),One)) = 2 * Join(add(u,v) + 1}.$

    \item[] $\mname{val(add(Join(u,One), Join(v,One))) = val(Join(add(add(u,v), Join(Nil, One)),Zero))} = {}$\\ 
      \hspace*{2ex}$ \mname{2 * val(Join(add(add(u,v), Join(Nil, One))))}$.

  \ei

We know that \mname{len(add(u, v)) $\le$ len(u) + len(v) $=$ n} and using Inductive hypothesis $P(0), P(1), ... , P(n-2), P(n-1)$, we know that \mname{val(add(a, b))} forms a bijection with $\mathbb{N}$ then P(n) takes 1 more step than P(n-1) therefore it is finite and $P(n)$ holds.

\medskip

Therefore, $P(n)$ holds for all $u, v \in$ \mname{BinNum} by strong induction.
\end{proof}
