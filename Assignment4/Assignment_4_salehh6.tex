...
Let $\Sigma_{\rm <}$ be the signature
    $(\sB,\emptyset,\emptyset,\sP,\tau)$  where:

  \be

    \item $\sB = \set{\alpha~|~\alpha \in \mathbb{R}}$.

    \item $\sP = \set{<}$
      where $\tau(<) = \alpha \times \alpha \rightarrow \mathbb{B}$.

  \ee

Let $\Gamma_{\rm [0,10]}$ be the set of axioms:

  \be

    \item (Irreflexitve) $\forall~x \in \sB~.~\lnot (x < x)$.

    \item (Asymmetric) $\forall~x,y \in \sB~.~(x < y) \implies \lnot (y < x)$.

    \item (Transitive) $\forall~x,y,z \in \sB~.~(x < y \land y < z) \implies (x < z)$.

    \item (Trichotomous) $\forall~x,y \in \sB~.~x < y \lor y < x \lor x = y$.

    \item (Dense) $\forall~x,y \in \sB~.~\exists~z \in \sB~.~ (x < z < y)$.

    \item (Max = 10) $\forall~x \in \sB~.~(x < 10) \lor (x = 10) $.

    \item (Min = 0) $\forall~x \in \sB~.~(0 < x) \lor (x = 0)$.

  \ee

Let $T_{\rm [0,10]} =(\Sigma_{\rm <}, \Gamma_{\rm [0,10]})$ be a theory in MSFOL.

Let $\sM_{\rm 1} = (\set{D_\alpha},I)$ and $\sM_{\rm 2} = (\set{D_{\beta}},I)$ where $\beta = \frac{\alpha}{2}$, be 2 models for $T_{\rm [0,10]}$.

Therefore, 
\[\set{d \in D_\alpha \;|\; V_{\phi[x : \alpha \mapsto d]}^{\sM_{1}}p\,(x:\alpha) = T}\]
and 
\[\set{d \in D_\beta \;|\; V_{\phi[y : \beta \mapsto d]}^{\sM_{2}}p\,(y:\beta) = T}\]
hold.
...
\begin{enumerate}[a.]
\item \verb+front(empty) = error+
\item \verb+front(enqueue(e, empty)) = e+
\item Let \verb+q = enqueue(e, q')+ where q' is non-empty, then \\
\verb+front(q) = front(q')+
\item \verb+back(empty) = error+
\item Let \verb+q = enqueue(e, q')+, then \\
\verb+back(q) = e+
\item \verb+dequeue(empty) = empty+
\item \verb+dequeue(enqueue(e, empty)) = empty+
\item Let \verb+q = enqueue(e, q')+, where q' is non-empty, then \\
\verb+dequeue(q) = enqueue(e, dequeue(q'))+
