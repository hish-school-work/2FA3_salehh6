...
Let $\Gamma_{\rm <}$ be the set of axioms:
  ...
  \be
    ...
    \item (Maximum) $\exists~M \in \sB~.~\forall~x \in \sB~.~ \lnot (M < x)  $.

    \item (Minimum) $\exists~m \in \sB~.~\forall~x \in \sB~.~ \lnot (x < m)  $.

  \ee

Let $T_{\rm <} =(\Sigma_{\rm <}, \Gamma_{\rm <})$ be a theory in MSFOL.

Let $\sM_{\rm 1} = (\set{\alpha~|~\alpha \in \mathbb{R}~.~0 \le \alpha \le 10},I)$ and since $\mathbb{Q} \subset \mathbb{R}$, let $\sM_{\rm 2} = (\set{\beta~|~\beta \in \mathbb{Q}~.~10 \le \beta \le 100},I)$, be 2 models for $T_{\rm <}$.
